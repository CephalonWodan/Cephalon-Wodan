...
async function fetchWorldstateText(platform) {
  ...
  if (!r.ok) {
    // Ajout d'un log en cas de réponse non 200
    const errText = await r.text().catch(() => '[no body]');
    console.error(`worldstate ${platform} HTTP ${r.status}`, errText.slice(0, 512));
    throw new Error(`worldstate ${platform} ${r.status}`);
  }
  const text = await r.text();
  if (!text || typeof text !== 'string') {
    console.error(`worldstate ${platform}: réponse vide ou invalide`, { text });
    throw new Error(`worldstate ${platform} empty-response`);
  }
  // Optionnel : log si le texte n'est pas du JSON (fréquent en cas de maintenance)
  if (!text.startsWith('{')) {
    console.warn(`worldstate ${platform}: réponse inattendue`, text.slice(0, 256));
  }
  return text;
}
...
// Correction : parseWorldstate utilise la classe exportée par défaut et le mot-clé new, avec vérif anti-undefined
async function parseWorldstate(text, platform) {
  if (!text || typeof text !== 'string') {
    console.error('parseWorldstate: texte worldstate vide ou invalide', { text });
    throw new Error(`parseWorldstate: texte worldstate vide ou invalide pour ${platform}`);
  }
  ...
  try {
    ...
    // Ajout d'un log pour inspecter le texte en cas d'erreur
    console.error('parseWorldstate: erreur lors du parsing', { text: text.slice(0, 512), err });
    throw err;
  }
}
