...
export const ALLOWED_SECTIONS = new Set([
  // Cycles
  "earthCycle", "cetusCycle", "vallisCycle", "cambionCycle", "duviriCycle", "zarimanCycle",
  // Activités
...
export async function fetchSection(platform, section, lang = "en") {
  ...
  // WarframeStatus préfère un slash final pour éviter 301
  ...
  return r.json(); // pass-through (array, object ou vide)
}
...
export async function fetchAggregated(platform, lang = "en") {
  const wanted = [
    ...
    "fissures", "alerts", "invasions", "nightwave", "sortie", "archonHunt",
    "voidTrader",
    // Décommente si tu veux inclure les primes dans l'agrégat racine :
    // "syndicateMissions",
  ];
