// lib/worldstate.js
// Worldstate — v5 bridge ++ : i18n multi-chemins, parseArray/parseAsyncArray, classe/fonction, ESM/CJS, cache 60s, meta debug
...
// ---------- i18n init (essaie plein de chemins) ----------
async function initLanguage(locale, steps) {
  ...
  try {
    ...
    if (typeof fn === 'function') {
      language = fn(locale);
      record(true, 'esm utilities.getLanguage(locale)');
    } else {
      record(false, 'esm utilities.getLanguage(locale)', 'not a function');
    }
  } catch (e) {
    record(false, 'esm import utilities', String(e?.message || e));
  }
  ...
  if (!language) {
    try {
      ...
      if (typeof fn === 'function') {
        language = fn(locale);
        record(true, 'esm main.getLanguage(locale)');
      } else {
        record(false, 'esm main.getLanguage(locale)', 'not a function');
      }
    } catch (e) {
      record(false, 'esm import main', String(e?.message || e));
    }
  }
  ...
  if (!language) {
    try {
      ...
      try {
        ...
        if (typeof fn === 'function') {
          language = fn(locale);
          record(true, 'cjs utilities.getLanguage(locale)');
        } else {
          record(false, 'cjs utilities.getLanguage(locale)', 'not a function');
        }
      } catch (e) {
        record(false, 'cjs require utilities', String(e?.message || e));
      }
      if (!language) {
        try {
          ...
          if (typeof fn === 'function') {
            language = fn(locale);
            record(true, 'cjs main.getLanguage(locale)');
          } else {
            record(false, 'cjs main.getLanguage(locale)', 'not a function');
          }
        } catch (e) {
          record(false, 'cjs require main', String(e?.message || e));
        }
      }
    } catch {/* no createRequire */}
  }

  // 3) optional setter (certaines builds lisent un global)
  if (language) {
    try {
      ...
      if (typeof setLang === 'function') {
        setLang(language);
        record(true, 'setLanguage(language)');
      } else {
        record(false, 'setLanguage(language)', 'setter not found');
      }
    } catch (e) {
      record(false, 'import translation', String(e?.message || e));
    }
  }
...
// ---------- parse v5 : parseArray/parseAsyncArray -> classe -> default fn (ESM puis CJS) ----------
