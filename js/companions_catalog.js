...
(() => {
  ...
  function injectAttacksFromLua(standardList, luaList){
    ...
    for (const item of standardList){
      ...
      if (src && Array.isArray(src.Attacks)) item.Attacks = src.Attacks;
  ...
  /* ----------------- MOA/Hound depuis ExportWeapons ----------------- */
    ...
    for (const it of arr){
      ...
      if (/\/MoaPetParts\//i.test(u)) {
        const slot = /MoaPetHead/i.test(u) ? "Head"
                   ...
                   : "Other";
        moa[slot].push(decoratePart(it, name, desc));
        continue;
      }
      ...
      if (/\/ZanukaPetParts\//i.test(u)) {
        const slot = /ZanukaPetPartHead/i.test(u) ? "Head"
                   ...
                   : "Other";
        hound[slot].push(decoratePart(it, name, desc));
        continue;
      }
    }
  ...
  function decoratePart(it, name, desc){
  ...
  function attacksBlock(item){
    ...
    return `<div class="mt-4"><div class="text-sm muted mb-1">Attaques</div><div class="bg-[var(--panel-2)] rounded-xl p-3 border border-[rgba(255,255,255,.08)]">${lines.map(l=>`<div class="py-1">${l}</div>`).join("")}</div></div>`;
  }
