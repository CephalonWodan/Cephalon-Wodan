...
let dbg;
function dbgInit(){ if(!DEBUG) return; dbg=document.createElement('div'); dbg.id='hub-debug'; dbg.innerHTML='<div class="hd-head">Hub Debug</div><div class="hd-body"></div>'; document.body.appendChild(dbg); }
function dbgLog(lines){ if(!DEBUG||!dbg) return; const b=dbg.querySelector('.hd-body'); b.innerHTML=Array.isArray(lines)?lines.join('<br/>'):String(lines); }
...
function fmtETA(ms){
  ...
  const parts=[]; if(d>0) parts.push(`${d}j`); if(h>0) parts.push(`${h}h`); if(m>0) parts.push(`${m}m`); parts.push(`${ss}s`); return parts.join(' ');
}
...
async function fetchAgg(platform, lang){
  const p=normPlatform(platform), l=normLang(lang);
  const url=`${API_BASE}/api/${p}?lang=${encodeURIComponent(l)}`; if(DEBUG) console.log('[HUB] fetch',url);
...
function renderCycles(data){ if(!els.cyclesList) return;
  ...
  let n=0; for(const [name,c,st] of rows){ if(!c||!c.expiry) continue; n++; const li=createEl('li','wf-row'); const L=createEl('div','left'); L.append(createEl('span','inv-node',name)); L.append(createEl('span','wf-badge',st(c))); const R=createEl('div','right'); R.append(makeEta(c.expiry,'')); li.append(L,R); els.cyclesList.append(li); }
...
function renderFissures(data){ if(!els.fissuresList) return;
  const all=Array.isArray(data?.fissures)?data.fissures:[]; const list=filterFiss(all); els.fissuresList.innerHTML=''; if(els.ctxFissures) els.ctxFissures.textContent=`${list.length}/${all.length} actives`;
  for(const f of list){ const li=createEl('li','wf-row'),L=createEl('div','left'),R=createEl('div','right');
    L.append(createEl('span',`wf-chip tier-${(f.tier||'').toLowerCase()}`,f.tier||'—')); L.append(createEl('span',`wf-chip ${f.isHard?'tag-hard':'tag-normal'}`,f.isHard?'Steel Path':'Normal')); L.append(createEl('span','wf-chip',f.missionType||'—'));
    R.append(createEl('span','muted',f.node||'—')); R.append(makeEta(f.expiry,'')); li.append(L,R); els.fissuresList.append(li); }
}
function renderSortie(data){ if(!els.sortie) return; els.sortie.innerHTML=''; const s=data?.sortie; if(!s) return;
  const head=createEl('div','inv-head'); head.append(createEl('span','inv-node',s.boss||'Sortie')); head.append(createEl('span','wf-badge',s.faction||'—')); head.append(makeEta(s.expiry,'')); const box=createEl('div','sortie-variants');
  (s.variants||[]).forEach(v=>{ const r=createEl('div','wf-row'),L=createEl('div','left'); L.append(createEl('span','wf-chip',v.missionType||v.type||'—')); if(v.modifier)L.append(createEl('span','wf-chip',v.modifier)); if(v.node)L.append(createEl('span','wf-chip',v.node)); r.append(L); box.append(r); }); els.sortie.append(head,box);
}
function renderArchon(data){ if(!els.archon) return; els.archon.innerHTML=''; const a=data?.archonHunt; if(!a) return;
  const head=createEl('div','inv-head'); head.append(createEl('span','inv-node',a.boss||'Archon Hunt')); head.append(createEl('span','wf-badge',a.faction||'—')); head.append(makeEta(a.expiry,'')); const box=createEl('div','sortie-variants');
  (a.missions||[]).forEach(m=>{ const r=createEl('div','wf-row'),L=createEl('div','left'); L.append(createEl('span','wf-chip',m.type||'—')); if(m.node)L.append(createEl('span','wf-chip',m.node)); r.append(L); box.append(r); }); els.archon.append(head,box);
}
function renderDuviri(data){ if(!els.duviri) return; els.duviri.innerHTML=''; const d=data?.duviriCycle||{}; const choices=Array.isArray(d.choices)?d.choices:[]; const normal=choices.find(c=>(c.category||'').toLowerCase()==='normal'); const hard=choices.find(c=>(c.category||'').toLowerCase()==='hard');
  const wrap=createEl('div','circuit-wrap'); const grp=(label,arr)=>{ const g=createEl('div','circuit-group'); g.append(createEl('div','circuit-title',label)); const chips=createEl('div','chips'); (arr||[]).forEach(n=>chips.append(createEl('span','wf-chip',n))); if(!arr||!arr.length) chips.append(createEl('span','muted','—')); g.append(chips); return g; };
  wrap.append(grp('Normal',normal?.choices||[])); wrap.append(grp('Steel Path',hard?.choices||[])); els.duviri.append(wrap);
  const n=(normal?.choices||[]).length, h=(hard?.choices||[]).length; if(els.ctxDuviri) els.ctxDuviri.textContent=[n?`${n} normal`:null,h?`${h} hard`:null].filter(Boolean).join(' • ')||'—';
}
function renderNightwave(data){ if(!els.nightwaveList) return; const host=els.nightwaveList; host.innerHTML=''; const nw=data?.nightwave;
...
function renderBaro(data){ if(!els.baroStatus||!els.baroInv) return; els.baroStatus.innerHTML=''; els.baroInv.innerHTML='';
  const b=data?.voidTrader; if(!b){ els.baroStatus.textContent='Baro non disponible'; return; }
  const arriving=(new Date(b.activation).getTime()-Date.now())>0; const p=createEl('p'); p.textContent=arriving?`Arrive à ${b.location||'—'} dans `:`Présent à ${b.location||'—'}, part dans `; p.append(makeEta(arriving?b.activation:b.expiry,'')); els.baroStatus.append(p);
...
function renderInvasions(data){ if(!els.invList) return; const list=Array.isArray(data?.invasions)?data.invasions:[]; els.invList.innerHTML=''; if(els.ctxInv) els.ctxInv.textContent=`${list.length} actives`;
  if(!list.length){ els.invList.append(createEl('li','muted','Aucune invasion active')); return; }
  list.forEach(v=>{ const li=createEl('li','wf-row'),L=createEl('div','left'),R=createEl('div','right');
    ...
    R.append(createEl('span','wf-bar__label', v.completed?'Terminé':`${pct.toFixed(2)}%`)); li.append(L,R); els.invList.append(li);
  });
}
...
function renderBounties(data){ if(!els.bounty) return;
...
async function loadAndRender(){
  try{
    ...
    const platform=normPlatform(els.platform?.value), lang=normLang(els.lang?.value);

    const missing=[]; for(const [k,v] of Object.entries(els)){ if(['now','platform','lang','fTier','fHard','grid'].includes(k)) continue; if(v==null) missing.push(`#${k.replace(/[A-Z]/g,m=>'-'+m.toLowerCase())}`); }

    const agg=await fetchAgg(platform,lang); LAST.agg=agg;
    ...
    if(DEBUG){
      const counts={ fissures:(agg.fissures||[]).length, invasions:(agg.invasions||[]).length, nightwave:(agg.nightwave?.activeChallenges||[]).length, sortie:agg.sortie?1:0, archonHunt:agg.archonHunt?1:0, baroInventory:(agg.voidTrader?.inventory||[]).length, syndicateMissions:(agg.syndicateMissions||[]).length };
      const lines=[`<b>platform/lang:</b> ${platform} / ${lang}`, `<b>fetch:</b> OK`, `<b>counts:</b> ${Object.entries(counts).map(([k,v])=>`${k}:${v}`).join(' | ')}`, missing.length?`<b>missing IDs:</b> ${missing.join(', ')}`:`<b>missing IDs:</b> none`];
      dbgLog(lines); console.table(counts); if(missing.length) console.warn('Missing containers:',missing);
    }
  }catch(e){
    console.error('hub load error',e);
    if(DEBUG) dbgLog([`<b>fetch:</b> ERROR`,`<b>message:</b> ${e&&e.message?e.message:e}`]);
  }
}
